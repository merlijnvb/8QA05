    ...
    def optimize(self, data={}, k_min=2, k_max=10, measure = 'Sil', seeds = []):
        ...
        for k in range(k_min, k_max+1): # LOOP OVER THE RANGE OF k'S THAT IS GIVEN
            ...
            if k_min != k_max: # IF k_min IS EQUAL TO k_max, ONLY ONE VALUE FOR k IS TESTED. IN THAT CASE, THE PROGRESS BAR IS NOT NECESSARY.
                print(f'{(((k-k_min)/(k_max+1-k_min))*100):.2f}%') # PRINT AT EVERY ITERATION HOW FAR THE EVALUATION PROCES IS
...
class GBCA:
    ...
    def optimize(self, data={}, subspace_min=10, subspace_max=20, measure = 'Sil'):
        ...
        for subspace in range(subspace_min, subspace_max+1): # LOOP OVER THE RANGE OF SUBSPACES THAT IS GIVEN
            ...
            print(f'{(((subspace-subspace_min)/(subspace_max+1-subspace_min))*100):.2f}%') # PRINT AT EVERY ITERATION HOW FAR THE EVALUATION PROCES IS
...
def main(filename):
    ...
    method = str(input('Which clustering algorithm do you want to use?\n"KMCA" or "GBCA": ')).upper()
    ...
    if method =='KMCA':
         ...
         kmca_silscores = kmca.lib_silscores
         outfile_name = 'kmca_results.txt'
         return_txt_file(kmca_results, outfile_name, lib_data)
         return_txt_file(kmca_silscores, 'kmca_silscores.txt', lib_data)
    ...
    if method == 'GBCA':
        ...
        gbca_silscores = gbca.lib_silscores
        outfile_name = 'gbca_results.txt'
        return_txt_file(gbca_results, outfile_name, lib_data)
        return_txt_file(gbca_results, outfile_name, lib_data)
        return_txt_file(gbca_silscores, 'gbca_silscores.txt', lib_data)
