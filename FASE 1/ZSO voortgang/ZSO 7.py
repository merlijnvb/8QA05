...
def dict_to_txt(rDict,filename,parse):
    ...
    outstring = "" # the string that will be filled with all information
    
    for ID in rDict:
        outstring += str(ID) 
        for r in rDict[ID]: outstring += parse + str(r)
        outstring+="\n"
...
def plot_phase1(Dagen,rDict,r_filter=0.5):
    ...
    bar_plots(rDict,r_filter) # to plot the bar plots
...
def bar_plots(rDict,r_filter,movement=256):
...
def Main():
    ...
    dict_to_txt(Filtered_rDict,"clusterdata.txt","  ")  # make a file of filtered data for phase 2
    dict_to_txt(rDict,"unfiltered_clusterdata.txt","  ")# make a file of unfiltered data for phase 3
    if input("Would you like the plots of phase 1?\n"): # ask whether the user would like to see the plots of phase 1 (boolean input)
        ...
        # plot_phase1(Dagen,Filtered_rDict)               # plot everything there is to plot in phase 1 based on the filtered data
